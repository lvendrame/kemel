...
namespace Kemel.Orm.Providers.MySQL
{
    ...
    {
        /// <summary>
        /// MySQL Connection String omiting tnsnames.ora
        ...
        /// </summary>
        public const string MySQLInformationSchema = "information_schema";
        public const string MySQLSystemSchemas = "('performance_schema', 'information_schema', 'mysql', 'test')";
        public const string MySQLConnectionString = "Server={0};Port={1};Database={2};Uid={3}; Pwd={4};";
        ...
        public override QueryBuilder QueryBuilder
        {
            get
            {
                if (this.qrbQueryBuilder == null)
                    this.qrbQueryBuilder = new MySQLQueryBuilder(this);
        ...
        {
            get { return new MySQLEntityCrudBuilder(this); }
        }
        ...
        public override string TableList
        {
            get
            {
                ...
                stb.Append(" SELECT TABLE_NAME AS NAME ");
                stb.Append("   FROM TABLES ");
                stb.Append("  WHERE TABLE_SCHEMA = '").Append(this.Credential.Catalog.Trim()).Append("'");
                stb.Append("  ORDER BY TABLE_NAME ");
                return stb.ToString();
            }
        }
        ...
        public override string TableDefinition
        {
            get
            {
                ...
                stb.AppendFormat("      T.TABLE_NAME '{0}',", T_Column_Entity.Definition.TABLE);
                ...
                stb.Append("       FROM TABLES T ");
                stb.Append("      INNER JOIN COLUMNS C ON C.TABLE_SCHEMA = T.TABLE_SCHEMA ");
                stb.Append("        AND C.TABLE_NAME = T.TABLE_NAME ");
                stb.Append("      WHERE T.TABLE_TYPE = 'BASE TABLE' ");
                stb.Append("        AND T.TABLE_SCHEMA = '").Append(this.Credential.Catalog.Trim()).Append("'");
                stb.Append("        AND T.TABLE_NAME = ?tableName ");
                stb.Append("      ORDER BY T.TABLE_NAME, C.ORDINAL_POSITION ");
                return stb.ToString();
            }
        }
        ...
        public override string ViewDefinition
        {
            get
            {
                ...
                stb.AppendFormat("      NULL '{0}',", T_Column_Entity.Definition.TABLE_REF);
                stb.AppendFormat("      NULL '{0}'", T_Column_Entity.Definition.COLUMN_REF);
                stb.Append("       FROM VIEWS V ");
                stb.Append("      INNER JOIN COLUMNS C ON C.TABLE_SCHEMA = V.TABLE_SCHEMA ");
                stb.Append("        AND C.TABLE_NAME = V.TABLE_NAME ");
                stb.Append("      WHERE V.TABLE_SCHEMA NOT IN ('performance_schema', 'mysql') ");
                stb.Append("        AND V.TABLE_NAME = ?tableName ");
        ...
        public override string AllDataBases
        {
            ...
            {
                StringBuilder stb = new StringBuilder();
                stb.Append(" SELECT schema_name ");
                stb.Append("   FROM SCHEMATA ");
                stb.Append("  WHERE schema_name NOT IN ").Append(MySQLSystemSchemas);
                stb.Append("  ORDER BY schema_name ");
                return stb.ToString();
            }
        }
        ...
        public override Kemel.Orm.Data.OrmConnection GetConnection()
        {
            Kemel.Orm.Data.OrmConnection connection =
                new Kemel.Orm.Data.OrmConnection(
                    new MySql.Data.MySqlClient.MySqlConnection(this.GetConnectionString())
                );
            return connection;
        }
    }
