...
namespace Kemel.Orm.Providers.Postgres
{
    ...
    {
        public PostgresQueryBuilder(Provider parent) : base(parent)
        {
        ...
        public override void WriteLimitEnd(SelectWriter writer, Kemel.Orm.NQuery.Storage.Query query)
        {
            writer.EndLimit.Append("LIMIT").Append(Punctuation.WHITE_SPACE)
                .Append((query.LimitRows.End + 1) - query.LimitRows.Start).Append(Punctuation.WHITE_SPACE)
                .Append("OFFSET").Append(Punctuation.WHITE_SPACE).Append(query.LimitRows.Start);
        }
